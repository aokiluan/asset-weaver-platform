...
export function RegistrarContatoDialog({ open, onOpenChange, contact, onSaved }: Props) {
  const [date, setDate] = useState(todayISO());
  const [type, setType] = useState<InvestorActivityType>("ligacao");
  ...
  async function handleSave() {
    ...
    // Atualiza último contato se mais recente
    if (!contact.last_contact_date || date > contact.last_contact_date) {
      await supabase
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          {contact && (
            <p className="text-[11px] text-muted-foreground leading-tight">{contact.name}</p>
          )}
        </DialogHeader>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="space-y-1">
              ...
              <Select value={type} onValueChange={(v) => setType(v as InvestorActivityType)}>
