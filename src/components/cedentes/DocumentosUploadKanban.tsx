...
export function DocumentosUploadKanban({
  ...
  const grupos = useMemo(() => {
    ...
    for (const c of categorias) {
      ...
      const todosDocs = documentos.filter((d) => d.categoria_id === c.id);
      // Em "Pendentes", inclui também categorias obrigatórias ainda sem nenhum upload.
      if (filter === "pendentes" && docs.length === 0) {
        if (!c.obrigatorio || todosDocs.length > 0) continue;
      } else if (filter !== "todos" && filter !== "pendentes" && docs.length === 0) {
        continue;
      }
